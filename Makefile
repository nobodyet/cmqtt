...
INCLUDE_PATH = -I./include  -I./mqttlib/ 
#编译参数
...
MYLIBS = -Wl,-E  -lrt -lpthread -lm -lz -ldl  -L/usr/lib64/mysql  -lmysqlclient


PWD_PATH =$(shell pwd)
#通用lib库
COMMON_LIB =   libcommon.a   
COMMON_LIB_PATH = $(PWD_PATH)/lib/

#通用lib库
CORE_LIB = libcore.so
CORE_LIB_PATH = $(PWD_PATH)/core/

#MQTT 相关文件
MQTT_LIB = libpaho-mqtt3a.so
MQTT_LIB_PATH = $(PWD_PATH)/mqttlib/
...
$(TARGET): ./src/main.o   $(MQTT_LIB) $(CORE_LIB)  $(wildcard $(CORE_LIB_PATH)/*.c) $(wildcard $(CORE_LIB_PATH)/*.h)
	...
	$(CC) -o $(TARGET) $(CFLAGS) $(MYLIBS) ./src/main.o $(COMMON_LIB_PATH)$(COMMON_LIB) $(CORE_LIB_PATH)$(CORE_LIB) $(MQTT_LIB_PATH)$(MQTT_LIB)
	$(CP) $(TARGET) $(RUN_PATH)
	$(CP) $(MQTT_LIB_PATH)$(MQTT_LIB) $(RUN_PATH)
	$(CP) $(CORE_LIB_PATH)$(CORE_LIB) $(RUN_PATH)
	$(CP) $(COMMON_LIB_PATH)$(COMMON_LIB) $(RUN_PATH)
	$(RM) $(TARGET)
	@echo -e "\\033[32m $(TAEGET) is OK and move to Dir $(RUN_PATH)  \\033[0m"
